...
<template>
    ...
    <DashboardLayouts :auth="page.props.auth">
        ...
        <a href="/dashboard/payment/"
            ...
            <span class="text-base sm:text-lg group-hover:underline">Kembali ke Daftar Pembayaran</span>
        </a>
        <div class=" min-h-screen ">
            <div class="mx-auto  sm:max-w-3xl">
                <div
                    class="bg-white shadow-2xl rounded-2xl p-4 sm:p-6 md:p-10 border border-indigo-200 relative overflow-hidden">
                    <div class="absolute top-0 right-0 opacity-10 pointer-events-none select-none">
                        <i class="bx bx-receipt text-[6rem] sm:text-[10rem] md:text-[12rem] text-indigo-200"></i>
                    </div>
                    <h1
                        class="text-2xl  md:text-4xl font-extrabold mb-6 sm:mb-10 text-start md:text-center text-blue-500 tracking-wide drop-shadow flex items-center justify-center gap-2">
                    ...
                    <div class="space-y-6 sm:space-y-8">
                        <div class="flex items-center gap-3 sm:gap-4">
                            <div class="bg-indigo-50 rounded-full p-2 sm:p-3">
                                <i class="bx bx-hash text-xl sm:text-2xl text-indigo-700"></i>
                            </div>
                            <div>
                                <div class="text-gray-500 font-medium text-sm sm:text-base">ID Transaksi</div>
                                <div class="font-semibold text-indigo-700 text-base sm:text-lg">{{ payment.id }}</div>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-2 sm:gap-4 items-center text-sm sm:text-base">
                            <span class="text-gray-500 font-medium flex items-center gap-2">
                                <i class="bx bx-money text-green-600"></i> Jumlah
                            </span>
                            <span class="text-gray-700 font-bold text-base sm:text-lg text-right">Rp {{
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:gap-4 items-center text-sm sm:text-base">
                            ...
                            <span class="text-red-500 font-bold text-base sm:text-lg text-right">Rp {{
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:gap-4 items-center text-sm sm:text-base">
                            ...
                            <span class="text-blue-700 font-bold text-base sm:text-lg text-right">
                                Rp {{
                                    (
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:gap-4 items-center text-sm sm:text-base">
                            ...
                            <span v-if="payment && payment.payment_status"
                                class="justify-self-end w-fit px-3 sm:px-4 py-1 rounded font-semibold text-right inline-block shadow"
                                :class="{
                            ...
                            <span v-else
                                class="justify-self-end w-fit px-3 sm:px-4 py-1 rounded font-semibold text-right inline-block text-gray-400 bg-gray-100 shadow">
                                Status tidak tersedia
                            </span>
                        </div>
                        <div v-if="payment.payment_photo" class="flex flex-col items-center pt-6 sm:pt-8">
                            <span class="text-gray-500 mb-2 text-sm sm:text-base font-medium flex items-center gap-2">
                                <i class="bx bx-image-alt text-indigo-600"></i> Bukti Pembayaran
                            </span>
                            <div class="relative group">
                                <img :src="payment.payment_photo" alt="Bukti Pembayaran"
                                    class="rounded-2xl max-h-40 sm:max-h-48 w-auto border-4 border-indigo-200 shadow-lg cursor-pointer transition-transform duration-200 group-hover:scale-105 hover:shadow-2xl"
                                    @click="openPreview" />
                                <span
        ...
        <transition name="fade">
            <div v-if="previewOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70"
                ...
                <div
                    class="bg-white rounded-2xl p-4 sm:p-8 shadow-2xl max-w-xs sm:max-w-2xl md:max-w-3xl w-full flex flex-col items-center animate-fadeIn">
                    <img :src="payment.payment_photo" alt="Preview Bukti Pembayaran"
                        class="max-h-[50vh] sm:max-h-[70vh] w-auto mb-4 sm:mb-6 rounded-xl border-4 border-indigo-200 shadow-lg" />
                    <button
                        class="px-6 sm:px-8 py-2 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition"
                        @click="closePreview">
                        Tutup
                    </button>
                </div>
