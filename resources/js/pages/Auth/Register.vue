...
// === Real-time validation using watchers ===
watch(() => form.value.email, (val) => {
    if (!val) errors.value.email = 'Email wajib diisi';
    else errors.value.email = '';
});

watch(() => form.value.username, (val) => {
    if (!val) errors.value.username = 'Username wajib diisi';
...
watch(() => form.value.password, (val) => {
    ...
    // Update konfirmasi juga kalau password berubah
    if (form.value.password_confirmation && form.value.password_confirmation !== val) {
        errors.value.password_confirmation = 'Password tidak cocok';
    } else {
...
watch(() => form.value.phone, (val) => {
    if (!val) {
        errors.value.phone = 'Nomor HP wajib diisi';
    } else if (!/^(\+62|0)[0-9]{9,13}$/.test(val)) {
        errors.value.phone = 'Format nomor HP tidak valid';
    } else {
        errors.value.phone = '';
    }
    ...
    <div class="bg-[#EDF3FF] min-h-screen text-indigo-900/75 flex items-center">
        <div
            ...
            <div class="md:flex-1 p-10 md:p-28 flex flex-col gap-4 justify-center md:max-w-1/2 ">
                <h1>
                    <a href="/register" class="text-2xl font-bold text-blue-900">.Taman Sari Kost</a>
                </h1>
                <div>
                        ...
                        <div class="flex justify-between gap-2">
                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="relative">
                                    ...
                                    <input v-model="form.password_confirmation" placeholder="konfirmasi password"
                        ...
                        <div class="flex flex-col">
                            ...
                            <div class="relative">
                                ...
                                <input v-model="form.phone" type="text" id="phone"
                                    class="p-2 px-8 border w-full rounded-md" placeholder="masukkan no telepon anda" />
                            </div>
